...
function useTypingEffect(words, speed = 90, pause = 2000) {
  ...
  useEffect(() => {
    ...
    let t;
    if (!del && ci <= cur.length) t = setTimeout(() => { setText(cur.slice(0, ci)); setCi(v => v + 1); }, speed);
    else if (!del) t = setTimeout(() => setDel(true), pause);
    else if (del && ci >= 0) t = setTimeout(() => { setText(cur.slice(0, ci)); setCi(v => v - 1); }, speed / 2);
...
export default function App() {
  ...
  // Pipeline logic moved to DeploymentPipeline3D
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <section id="work" className="py-12">
        <main className="bento-grid" ref={gridRef}>
          ...
          <BentoCard gridCol="1 / 2" gridRow="1 / 2" title="System Health" icon={<Cpu size={12} />}>
            <div className="flex flex-col gap-3 flex-1 justify-center">
              <div className="text-center">
                <div className={`text-4xl font-mono font-bold tabular-nums ${healthy ? 'text-green-400' : 'text-red-400'}`}>{load}%</div>
          ...
          <BentoCard gridCol="2 / 5" gridRow="1 / 2" title="Deployment Pipeline" icon={<Server size={12} />}>
            <DeploymentPipeline3D />
          </BentoCard>

          {/* TECH SUMMARY – col 3-5, row 2 */}
          <BentoCard gridCol="3 / 5" gridRow="2 / 3" title="Quick Stack" icon={<Cpu size={12} />}>
            <div className="grid grid-cols-3 gap-5 flex-1">
              {[
                ...
                <div key={lbl}>
                  <div className="text-[10px] font-bold uppercase tracking-widest mb-2.5" style={{ color }}>{lbl}</div>
                  <div className="space-y-1">
                    {items.map(it => <div key={it} className="text-[11px] text-gray-400 leading-snug">{it}</div>)}
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* ━━━━━━━━━━ ROW 3: Experience + 2 Projects ━━━━━━━━━━ */}

          {/* EXPERIENCE – col 1-2, row 2 */}
          <BentoCard gridCol="1 / 3" gridRow="2 / 3" title="Experience" icon={<Briefcase size={12} />}>
            <div className="flex flex-col gap-3 flex-1">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-base font-bold leading-snug">Web Development Intern</h3>
              ...
              <ul className="space-y-2 flex-1">
                {expBullets.map(b => (
                  <li key={b} className="flex items-start gap-2 text-[12px] text-gray-400 leading-relaxed">
            ...
            <div className="flex flex-col gap-4 flex-1">
              ...
              <div className="mt-auto pt-3 border-t border-white/5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-green-400 font-medium">Available for opportunities</span>
                </div>
              </div>
            </div>
          </BentoCard>
          ...
          <BentoCard gridCol="2 / 3" gridRow="4 / 5">
            ...
            <div className="flex flex-col gap-4 flex-1">
              {education.map((edu, i) => (
                <div key={edu.degree} className={`flex flex-col gap-1 ${i > 0 ? 'pt-3 border-t border-white/5' : ''}`}>
                  ...
                  <span className="text-[11px] text-gray-500">{edu.school}</span>
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-green-400 font-mono text-sm font-bold">{edu.cgpa}</span>
                    <span className="text-[10px] text-gray-600 font-mono">{edu.period}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <BentoCard gridCol="4 / 5" gridRow="4 / 5" className="bg-amber-900/10 border-amber-700/25 overflow-visible">
            <div className="flex flex-col items-center justify-center flex-1 text-center gap-3 relative">
              {/* Animated SVG steam */}
              <svg
              ...
              {/* 404 */}
              <div className="text-2xl font-bold font-mono text-amber-400 leading-none">404</div>
              <p className="text-xs text-amber-200/50 font-mono">sleep not found</p>
          ...
          <BentoCard gridCol="2 / 5" gridRow="5 / 6" title="Tech Stack" icon={<Cpu size={12} />}>
            <FloatingTechStack />
          </BentoCard>
                ...
                <div className="flex gap-2.5">
                  {[
                    ...
                    <a
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 hover:border-blue-500/40 text-xs text-gray-400 hover:text-white transition-all group"
                    >
      ...
      <footer className="text-center py-6 text-gray-600 text-[11px] border-t border-white/5 bg-black/60 backdrop-blur-md font-mono">
  ...
  return (
    <div className="flex flex-col gap-3 flex-1">
      ...
      <div className="flex flex-wrap gap-1 pt-3 border-t border-white/5">
        {tags.map(t => (
          <span key={t} className="px-1.5 py-0.5 text-[10px] font-mono rounded border border-white/8 text-gray-500">{t}</span>
        ))}
      </div>
    </div>
  );
